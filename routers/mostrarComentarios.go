...
/*ReadCometarios servira para leer todas las publicaciones*/
func ReadCometarios(w http.ResponseWriter, r *http.Request) {
	IDP := r.URL.Query().Get("id")
	if len(IDP) < 1 {
		http.Error(w, "Debe enviar el parametro id para poder ver cometarios de la publicacion", http.StatusBadRequest)
		return
	}
	...
	//pag := int64(page)
	respuesta, correct := bd.ReadComentarios(IDP)
	if correct == false {
